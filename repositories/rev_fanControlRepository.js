...
import { Gateway, GatewayManager } from '../GatewayManager.js';
...
const MQTT_CMD_TYPE = ["sta", "unit", "all"]; //sta:상태얻어오기, unit:개별제어, all:전체제어
...
//     //     topic = `FANZIC_CF/${gate.mac_adr}/dat`
//     //     console.log("topic =", topic);
//     //     const fan = await fanRepository.find({where: {gate_id: gate.id}});
//     //     connectedClientMsg = {type: "sta", unit_cnt: `0${fan.length}`};
//     //     connectedClientMsg = JSON.stringify(connectedClientMsg);
//     //     saveFanStatus(topic, connectedClientMsg);
//     // }
// }
...
async function saveFanStatus(topic, msg){
    //FANZIC_CF/1C:69:20:CE:F5:A8/dat
    ...
    try{
        ...
        //const str = '{"type":“sta",“unit_cnt":"□□"}';
        const escapedMsg = escapeJsonString(msg);
        const staMsg = JSON.parse(escapedMsg);

        //const staMsg = JSON.parse(msg);
        ...
        if (staMsg.type === "unit" && staMsg.motor_id !== "00" ) {
            ...
            gateWay.setFanStatus(staMsg.motor_id, staMsg.rpm, staMsg.dir, staMsg.status)
            return true;
        }

        if (staMsg.unit_cnt == undefined){
            //console.log("unit_cnt이 존재하지 않습니다.");
            return false;
        }

        console.log(staMsg); 
        var numList = generateNumberArray(staMsg.unit_cnt);
...
function escapeJsonString(jsonString) {
    ...
    //console.log("escapeJsonString=", replaceStr);
    ...
    return new Promise((resolve, reject) => {
        ...
        client.subscribe(sub_topic, (err) => {
            ...
            const runMessageHandler = (topic, message) => {
                ...
                if((staMsg.type === pubMsgJson.type) && (staMsg.motor_id === pubMsgJson.motor_id)){
                        msgList.push(staMsg)
                        //return false;
                }
                ...
                // client.removeListener('message', runMessageHandler);
                // // 응답 전송
                // //return { topic:topic, message: connectedClientMsg };
                // resolve({ topic:topic, message: connectedClientMsg });
                // return;
                //return true;
            };
